...
template <typename node_t> // node_t sarebbe index_t
std::vector<node_t> DegeneracyOrder(const graph_t<node_t>& graph) {
...
std::vector<node_t> RandomOrder(const graph_t<node_t>& graph) {
